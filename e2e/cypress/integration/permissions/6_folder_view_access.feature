...
Feature: Grant view access for a file
  ...
  Scenario: 3 User can not grand access for a folder to the user with incorrect email
    When The "User1" sends a request to grant "view" access to the "folder" "Transfer" to "incorrectemail@g.com"
  ...
  Scenario: 7 Owner can not grand access for a folder to some users
    ...
    When The user1 sends a request to grant "view" access to the "folder" "Transfer" to user2 and user3
    Then Response status 422
    And Message "User for sharing not found"

  @negative
  Scenario: 8 Owner can not grand access for a folder if field "email" is empty
    When The "User1" sends a request to grant "view" access to the "folder" "Transfer" with "empty" email
    Then Response status 422
    And Message "User for sharing not found"

  @negative
  Scenario: 9 Owner can not grand access for a folder if field "email" contain spaces
    When The "User1" sends a request to grant "view" access to the "folder" "Transfer" with "spaces in" email
    Then Response status 422
    And Message "User for sharing not found"

  @negative
  Scenario: 10 Owner can grand access for a folder if field "email" contain username
    When The "User1" sends a request to grant "view" access to the "folder" "Transfer" with "username in" email
    Then Response status 200

  @negative
  Scenario: 11 Editor can can not to transfer ownership for a folder
    Given Send request for create user3 and get token
    And The "User1" sends a request to grant "view" access to the "folder" "Transfer" to "User2"
    And Response status 200
    And "User1" is the owner of the folder
  ...
  Scenario: 13 Owner can not grand access for a folder if the parameter "email" is absent
    Given The user1 sends a request to grant "view" access to the "folder" "Transfer" without "email"
    Then Response status 422

  @negative
  Scenario: 14 Owner can not grand access for a folder if the parameter "permissions" is absent
    Given The user1 sends a request to grant "view" access to the "folder" "Transfer" without "permissions"
    Then Response status 422

  @negative
  Scenario: 15 Owner can not grand access for a folder if the parameter "hash" is absent
    Given The user1 sends a request to grant "view" access to the "folder" "Transfer" without "hash"
    Then Response status 422
