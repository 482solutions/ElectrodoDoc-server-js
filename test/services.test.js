...
import {CreateFolder, DownloadFile, GetFolder, Search, UploadFile,} from '../src/service/FileSystemService';
...
describe('Creating Folder', async () => {
    ...
    it('should return new folder to database', async () => {
        ...
        const folderFromDB = (await DB.getFolderByName(conn, folderName))[0];
        assert.equal(result.payload.folder, folderFromDB.parenthash);
    });
});
...
describe('Uploading files', async () => {
    ...
    it('should return new file to database', async () => {
        ...
        const fileFromDB = (await DB.getFileByName(conn, fileName))[0];
        assert.equal(result.payload.folder, fileFromDB.parenthash);
    });
});
